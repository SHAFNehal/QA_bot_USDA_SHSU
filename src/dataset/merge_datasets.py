...
def merge_datasets(
    ...
    Args:
        ...
        multiturn_multiplier: How many times to repeat multi-turn data
    ...
    if multiturn_data_path and Path(multiturn_data_path).exists():
        ...
        if multiturn_multiplier < 1:
            multiturn_multiplier = 1
        for _ in range(multiturn_multiplier):
            all_data.extend(multiturn_data)
        if multiturn_multiplier > 1:
            print(f"Repeated multi-turn dataset x{multiturn_multiplier} for better representation")
    else:
...
def main():
    ...
    parser.add_argument(
        "--multiturn_multiplier",
        type=int,
        default=1,
        help="Times to repeat multi-turn data (for better representation)"
    )
    ...
    merge_datasets(
        ...
        multiturn_multiplier=args.multiturn_multiplier,
